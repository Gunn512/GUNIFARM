...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!productId) {
    mainContainer.innerHTML =
      "<p>Không tìm thấy sản phẩm này. Vui lòng trở về trang danh sách.</p>";
    return;
  }
  ...
  fetch("product.json")
    ...
    .catch((error) => {
      ...
      mainContainer.innerHTML =
        "<p>Lỗi: Không thể tải thông tin sản phẩm. Vui lòng kiểm tra lại đường dẫn file JSON.</p>";
    });
  ...
  function renderProductDetails(product, allProducts) {
    ...
    mainContainer.innerHTML = `
            ...
            <div class="product-info-detail">
                <h1>${product.name}</h1>
                <p class="brand">Thương hiệu: <strong>${
                  product.brand
                }</strong></p>
                <p class="category">Loại: <strong>${
                  product.category
                }</strong></p>
                ...
                <div class="price-section">
                    <span class="unit-price" data-unit-price="${
                      product.unitPrice
                ...
                <p class="total-price">Tổng cộng: <span id="total-price-value">${formatCurrency(
                  product.unitPrice
                )}</span></p>
  ...
  function renderRelatedProducts(currentProduct, allProducts) {
    ...
    if (relatedProducts.length > 0) {
      ...
      relatedProductsSection.innerHTML =
        "<h2>Sản phẩm tương tự</h2><p>Chưa có sản phẩm tương tự.</p>";
    }
  }
