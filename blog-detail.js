document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log(
    "Dữ liệu blogPosts đã tải:",
    typeof blogPosts !== "undefined" ? blogPosts : "Chưa tải"
  );
  ...
  if (postId) {
    ...
    if (currentPost) {
      ...
      blogContentWrapper.querySelector(".blog-detail-content").innerHTML +=
        navigationHtml;
    } else {
      blogContentWrapper.innerHTML =
        '<div class="error-message">Không tìm thấy bài viết này.</div>';
      document.title = "Bài viết không tồn tại - Guni Farm";
      console.error(`Không tìm thấy bài viết với ID: ${postId}`);
    }
  } else {
    blogContentWrapper.innerHTML =
      '<div class="error-message">Vui lòng chọn một bài viết để xem.</div>';
