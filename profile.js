...
const renderPurchaseHistory = () => {
  ...
  const productsWithReviews = productsData.filter(
    (p) => p.reviews && p.reviews.length > 0
  );

  if (productsWithReviews.length === 0) {
    reviewListContainer.innerHTML =
      '<p style="text-align: center; color: #777; margin-top: 20px;">Chưa có lịch sử mua hàng.</p>';
    return;
  }
...
const initializePage = async () => {
  try {
    ...
    document.getElementById("product-list").innerHTML =
      '<p style="text-align: center; color: red; margin-top: 20px;">Không thể tải dữ liệu lịch sử mua hàng. Vui lòng thử lại sau.</p>';
  }
};
